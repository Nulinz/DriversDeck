                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex mb-4">
                                <button class="btn btn-link text-decoration-none fw-bold me-4 section-toggle active"
                                    data-target="approved-section" style="color: #333; border-bottom: 2px solid #007bff;">
                                    Approved
                                </button>
                                ...
                                <button class="btn btn-link text-decoration-none fw-bold section-toggle"
                                    data-target="pending-section" style="color: #666;">
                                    Pending
                                </button>
                            </div>
...
<td>
    ...
    onsubmit="return confirm('Are you sure you want to approve this driver?')">
        @csrf
        <button class="btn btn-success btn-sm p-1" title="Approve">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            // Custom filter for vehicle types
            $.fn.dataTable.ext.search.push(
                function (settings, data, dataIndex) {
                    var selectedValue = $('#vehicleFilter').val();
                    var coverOfVehicle = data[2] || ""; // COV column index

                    if (selectedValue === "") {
                        return true; // Show all if no filter selected
                    }

                    // Split by comma and get the first value, then trim whitespace
                    var firstValue = coverOfVehicle.split(',')[0].trim();

                    // Check if the first value matches the selected filter
    ...
    language: {
        ...
        info: "Showing _START_ to _END_ of _TOTAL_ drivers",
        infoEmpty: "Showing 0 to 0 of 0 drivers",
        zeroRecords: "No pending drivers found"
    },
    columnDefs: [
